/*
  ...
  1. Advanced Indexing Strategy
    ...
    - Covering indexes for reduced table lookups
...
-- Index for deadline-based filtering (without date arithmetic)
...
-- Index for expired tokens cleanup (without NOW() function)
...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(uds.total_applications, 0),
    COALESCE(uds.applied_count, 0),
    COALESCE(uds.followed_up_count, 0),
    COALESCE(uds.interview_count, 0),
    COALESCE(uds.offer_count, 0),
    COALESCE(uds.rejected_count, 0),
    COALESCE(uds.no_response_count, 0),
    COALESCE(uds.pending_followups, 0),
    COALESCE(uds.avg_response_time_days, 0),
    COALESCE((
      SELECT json_agg(
        json_build_object(
          'id', ja.id,
  ...
  -- Fallback query if user not in materialized view
  SELECT 
    COUNT(*)::bigint,
    ...
    AVG(EXTRACT(DAY FROM (updated_at - created_at)))::numeric,
    (
      SELECT json_agg(
        json_build_object(
          'id', ja.id,
  ...
  FROM job_applications
  WHERE user_id = p_user_id
  AND NOT EXISTS (SELECT 1 FROM user_dashboard_stats WHERE user_id = p_user_id)
