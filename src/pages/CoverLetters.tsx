...
import { Mail, Sparkles, Copy, Download, Send, Save, Zap, Target, Brain, MessageSquare } from 'lucide-react';
...
export function CoverLetters() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const generation = await generateContent(
        formData.selectedJobId || 'general',
        'cover-letter',
      ...
      if (generation) {
        setGeneratedLetter(generation.content);
  ...
  const handleSave = async () => {
    if (!currentGenerationId || !generatedLetter) return;

    try {
      await saveGeneration(currentGenerationId, generatedLetter, true);
    } catch (error) {
      // Error handled in hook
    }
  };
  ...
  const handleExport = () => {
    const filename = `cover-letter-${formData.companyName.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}.txt`;
    exportContent(generatedLetter, filename);
  };
      ...
      >
        <div>
          ...
          <p className="text-slate-400 mt-2 flex items-center space-x-2 text-sm lg:text-base">
            ...
            <span>Create personalized, compelling cover letters in seconds</span>
          </p>
        </div>
                ...
                <div className="flex items-center justify-center h-96 bg-dark-900/50 rounded-xl border border-slate-700/30">
                  <div className="text-center p-6">
                    ...
                    <p className="text-slate-400 max-w-sm text-sm">
                      Fill in the details and click "Generate" to create your personalized cover letter
                    </p>
                  </div>
                </div>
              )}
      ...
      >
        <Card className="bg-gradient-to-r from-primary-900/20 to-secondary-900/20 border border-primary-600/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm text-slate-300">
            <div>
              <h4 className="font-medium text-slate-200 mb-2">Structure:</h4>
              <ul className="space-y-1 text-slate-400">
                <li>• Opening with enthusiasm</li>
                <li>• Highlight relevant experience</li>
                <li>• Show company knowledge</li>
                <li>• Strong closing call-to-action</li>
              </ul>
            </div>
            <div>
              <h4 className="font-medium text-slate-200 mb-2">Content:</h4>
              <ul className="space-y-1 text-slate-400">
                <li>• Customize for each application</li>
                <li>• Use specific examples</li>
                <li>• Match keywords from job posting</li>
                <li>• Show personality and passion</li>
              </ul>
            </div>
            <div>
              <h4 className="font-medium text-slate-200 mb-2">Format:</h4>
              <ul className="space-y-1 text-slate-400">
                <li>• Keep it concise (1 page max)</li>
                <li>• Use professional language</li>
                <li>• Proper business letter format</li>
